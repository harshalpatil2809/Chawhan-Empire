...
import type { BudgetBand, ProjectImage, ProjectType, PropertyType } from '@/lib/types';
...
export default function QuoteForm() {
  ...
  const validate = () => {
    ...
    if (!isPhone(form.phone)) next.phone = 'Enter a 10-digit phone number we can reach you on.';
    if (!isEmail(form.email)) next.email = 'Enter a valid email address.';
    if (form.location.trim().length < 2) next.location = 'Tell us the city or area of the site.';
  ...
  const submit = async () => {
    ...
    try {
      ...
      setFormError('The enquiry did not go through. Try again, or call us directly.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    return (
      <div className="border border-concrete-dark bg-white p-8 text-center sm:p-12">
        ...
        <p className="mx-auto mt-3 max-w-md text-sm leading-relaxed text-ink-mute">
          Our team will contact you shortly, usually within one working day. If it is urgent,
          call the office and quote your name.
        </p>
